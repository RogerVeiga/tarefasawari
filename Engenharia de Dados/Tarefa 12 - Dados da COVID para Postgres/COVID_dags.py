...
URLS_COVID = {
   ...
   'covid_vaccinations': 'https://github.com/owid/covid-19-data/blob/master/public/data/vaccinations/vaccinations.csv',
}
...
dag6 =  DAG(dag_id=f"ingest_covid_vaccinations_dag",start_date=datetime(2021,1,1),schedule_interval=None, catchup=False)
      
download_task = COVID_DownloadFromSourceOperator(
   task_id=f"download_{os.path.basename(URLS_COVID['covid_vaccinations'])}", url=URLS_COVID['covid_vaccinations'],dag=dag6
)
